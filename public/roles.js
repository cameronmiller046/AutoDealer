...
(function () {
  ...
  function loadCall(){ if(window.ADCall||document.getElementById('ad-call-script')) return; var s=document.createElement('script'); s.id='ad-call-script'; s.src='/call.js'; s.async=true; document.body.appendChild(s); }
  function loadSummary(){ if(window.ADSummary||document.getElementById('ad-summary-script')) return; var s=document.createElement('script'); s.id='ad-summary-script'; s.src='/summary.js'; s.async=true; document.body.appendChild(s); }
  function init(){ loadTheme(); applyQueryRole(); if(!guard()) return; injectCSS(); renderNav(); mountViewAs(); mountBanner(); mountAccountSwitcher(); applyGates(); loadAI(); loadNotify(); loadCmdK(); loadWire(); loadClock(); loadCall(); loadSummary(); }
